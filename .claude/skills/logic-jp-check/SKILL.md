---
name: logic-jp-check
description: 文章・主張・論証・レポート・メール・仕様などが「論理として通っているか」と「日本語として明快か」を最小手数で点検する。「これ論理的？」「ロジックチェックして」「論証の穴を探して」「主張と根拠は合ってる？」「この説明が分かりにくい」「文章を簡潔にして」「テクニカルライティングとして直して」といった依頼で必ず使う。「この文章、翻訳っぽい／直訳調／訳語臭い／英語をそのまま訳したみたい／不自然な日本語」「カルクを直して」も必ず拾う。表記ゆれ・用語のゆれ・敬体常体の混在・文体の乱れも見る。下書きの推敲、議論の検証、説明文・仕様・提案の改善でも起動すること。
---

# ロジカルチェック（思考 × 日本語）

正しい結論 = ファクト × ロジック。それを誤解なく日本語に載せる。
前者を波頭亮『思考・論理・分析』、後者をJTCA『日本語スタイルガイド』（簡潔・明快・誤解されない）で点検する。
